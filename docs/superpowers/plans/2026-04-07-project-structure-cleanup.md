...
**Goal:** Refactor the portfolio codebase for consistent inline styles, full data-content separation, and no duplicate component logic — zero functionality changes.

**Architecture:** All content moves to `src/data/` files. All styling uses inline style objects (a minimal `<style>` tag remains in AboutPage only for `@keyframes`, shimmer pseudo-element, and a responsive media query). Shared patterns are extracted into shared components.

**Tech Stack:** React 18, React Router, Vite, inline styles
...
|---|---|---|
| `src/data/about.js` | Modify | Add `bio` export |
| `src/data/home.js` | Modify | Add `icon` key to each service object |
| `src/components/CaseStudyPage.jsx` | Create | Full-page wrapper for case study view (replaces duplicate block) |
| `src/components/CaseStudy.jsx` | Modify | Export `Thumbnail` component |
| `src/pages/Home/index.jsx` | Modify | Use `CaseStudyPage`; switch to icon-key lookup |
| `src/pages/PortfolioPage.jsx` | Modify | Use `CaseStudyPage`; import shared `Thumbnail` |
| `src/pages/AboutPage.jsx` | Rewrite | Inline styles; bio read from `data/about.js` |

No test suite in this project — verify each task by running `npm run dev` and checking the relevant page.

---

## Task 1: Add data to about.js and home.js

**Files:**
- Modify: `src/data/about.js`
- Modify: `src/data/home.js`

- [ ] **Step 1: Add `bio` export to `src/data/about.js`**

Insert this block at the top of the file, before the `stats` export:
...
- [ ] **Step 2: Add `icon` keys to services in `src/data/home.js`**

Replace the existing `services` export with:
...
```bash
git add src/data/about.js src/data/home.js
git commit -m "feat: add bio to about.js, add icon keys to home services"
```

---

## Task 2: Create CaseStudyPage wrapper component

**Files:**
- Create: `src/components/CaseStudyPage.jsx`

Both `Home/index.jsx` and `PortfolioPage.jsx` contain an identical block that renders a case study in a full-page wrapper. Extract it once.
...
```bash
git add src/components/CaseStudyPage.jsx
git commit -m "feat: extract CaseStudyPage wrapper component"
```

---

## Task 3: Export Thumbnail from CaseStudy.jsx

**Files:**
- Modify: `src/components/CaseStudy.jsx`

`Thumbnail` currently lives as a local function inside `PortfolioPage.jsx`. Move it next to `PlaceholderThumb` so both grid pages import from one file.

- [ ] **Step 1: Add `Thumbnail` export to `src/components/CaseStudy.jsx`**

Insert this function immediately after the closing `}` of `PlaceholderThumb` (after line 38):
...
`colors` is already imported at the top of `CaseStudy.jsx` — no new imports needed.

- [ ] **Step 2: Commit**

```bash
git add src/components/CaseStudy.jsx
git commit -m "feat: export Thumbnail from CaseStudy"
```

---

## Task 4: Update Home/index.jsx

**Files:**
- Modify: `src/pages/Home/index.jsx`

Two changes: use `CaseStudyPage` for the selected-project view; switch `SERVICE_ICONS` from title-string keys to icon-string keys.

- [ ] **Step 1: Add CaseStudyPage import**
...
- [ ] **Step 2: Update SERVICE_ICONS map**
...
- [ ] **Step 3: Update SERVICE_ICONS lookup in the ServiceCard map**

Find the `homeServices.map` block and change `SERVICE_ICONS[s.title]` to `SERVICE_ICONS[s.icon]`:
...
- [ ] **Step 4: Replace the inline case-study wrapper**

Replace this block (currently lines 28–36):
```jsx
if (selectedProject) {
  return (
    <div style={{ background: colors.bg, minHeight: "100vh", fontFamily: fonts.body, padding: "clamp(24px, 5vw, 60px)" }}>
...
- [ ] **Step 5: Remove now-unused CaseStudy import**

`CaseStudy` is no longer imported directly in this file. Remove it from the import block (it was on line 9).

- [ ] **Step 6: Verify**

Run `npm run dev`. On the Home page: click a project card — case study should open; back button returns to Home. "What I Do" service icons should still render.

- [ ] **Step 7: Commit**

```bash
git add src/pages/Home/index.jsx
git commit -m "refactor: use CaseStudyPage and icon-key lookup in Home"
```

---

## Task 5: Update PortfolioPage.jsx

**Files:**
- Modify: `src/pages/PortfolioPage.jsx`

Two changes: use `CaseStudyPage`; remove local `Thumbnail` and import the shared one.

- [ ] **Step 1: Update imports**

Replace:
```js
import CaseStudy, { PlaceholderThumb } from "../components/CaseStudy.jsx";
```
...
- [ ] **Step 2: Delete the local Thumbnail function**

Remove lines 17–31 (the local `Thumbnail` function):
```jsx
/* ── Card thumbnail (grid) ── */
...
- [ ] **Step 3: Replace the inline case-study wrapper**

Replace this block (currently lines 73–81 after the deletion above shifts lines):
```jsx
if (selectedProject) {
  return (
    <div style={{ background: colors.bg, minHeight: "100vh", fontFamily: fonts.body, padding: "clamp(24px, 5vw, 60px)" }}>
...
- [ ] **Step 4: Remove unused fonts import**

Check whether `fonts` is still referenced in the file. Search for `fonts.body` — if no remaining usages exist, remove `fonts` from the tokens import:
```js
...
Run `npm run dev`. On the Portfolio page: filter tabs should work; clicking a project card opens the case study; back button returns to the grid; thumbnails display correctly.

- [ ] **Step 6: Commit**

```bash
git add src/pages/PortfolioPage.jsx
git commit -m "refactor: use CaseStudyPage and shared Thumbnail in PortfolioPage"
```

---

## Task 6: Rewrite AboutPage.jsx to inline styles

**Files:**
- Modify: `src/pages/AboutPage.jsx`

The biggest change. The ~340-line `<style>` string is replaced with inline style objects. A minimal `<style>` tag (~20 lines) remains for:
- `@keyframes btnShimmer` and `@keyframes statReveal` (cannot be done inline)
- The shimmer `::after` pseudo-element on the CTA button
- One `@media` breakpoint for the two-column → one-column grid
- `stat-reveal-N` classes used by the keyframe animation

The bio text is read from `data/about.js` instead of being hardcoded.

Hover effects (skill pills, CTA) use `onMouseEnter`/`onMouseLeave` handlers that set `e.currentTarget.style.*` directly — same pattern as `CaseStudy.jsx`'s back button.

The decorative `::before`/`::after` blobs on the section become `<div>` elements with `position: absolute`.

The stat divider (originally a CSS sibling `::before`) becomes a `position: absolute` child `<div>` inside each stat item when `i > 0`.

- [ ] **Step 1: Replace the full file with the following**
...
export default function AboutSection() {
  ...
  // Fade + slide-up reveal driven by `visible` state.
  // Uses separate opacity/transform transitions so the delay only affects
  // the initial reveal — hover handlers override transition imperatively.
  ...
  return (
    <>
      {/* Minimal style: keyframes + shimmer pseudo-element + responsive grid */}
      <style>{`
      ...
      >
        {/* Decorative glow — top right (replaces ::before pseudo-element) */}
        <div style={{
        ...
        {/* Decorative glow — bottom left (replaces ::after pseudo-element) */}
        <div style={{
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              {/* Gradient border accent (replaces ::before pseudo-element) */}
              <div style={{
            ...
            <div>
              ...
              {/* CTA — shimmer effect kept via .about-cta::after keyframe in the <style> block above */}
              <Link
                to="/resume"
                className="about-cta"
                style={{
          ...
          >
            {STATS.map((s, i) => (
              <div key={s.label} style={{ textAlign: "center", padding: "0 24px 35px", position: "relative" }}>
                {/* Vertical divider between items (replaces CSS sibling ::before selector) */}
                {i > 0 && (
                  <div style={{
...
- Image panel slides in from the left on load
- Label, heading, bio, skill pills, and CTA fade up with stagger
- Skill pills glow and lift on hover
- CTA button shimmers on hover
- Stats counter animates on scroll into view
- Stat divider lines appear between the four items
- On a narrow window (<768px) the grid collapses to a single column

- [ ] **Step 3: Commit**

```bash
git add src/pages/AboutPage.jsx
git commit -m "refactor: rewrite AboutPage to inline styles, read bio from data/about.js"
```
...
Run `npm run dev` and spot-check all five routes — Home, Portfolio, About, Resume, Contact — confirming no console errors and no visual regressions.
